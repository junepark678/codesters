...
p1 = codesters.Sprite(p1_img, -225, 0)
p1.set_size(0.15)
p1.turn_right(90)
p1.lives = 3
...
p2 = codesters.Sprite(p2_img, 210, 0)
p2.set_size(0.3)
p2.turn_left(90)
p2.lives = 3
...
def left_collision(sprite, hit_sprite):
    if hit_sprite.get_color() is "yellow" or hit_sprite.get_color() is "red":
        ...
        if p1.lives == 0:
            display.set_text("Dark Destroyer (Player 1) wins! >>>>>")
            end_game()
def right_collision(sprite, hit_sprite):
    if hit_sprite.get_color() is "blue" or hit_sprite.get_color() is "red":
        ...
        if p2.lives == 0:
            display.set_text("<<<<< Lit Destroyer (Player 2) wins!")
            end_game()
